...
<div class="space-y-6">
    <div class="rounded-3xl border border-slate-200 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-800 px-6 py-8 text-white shadow-lg">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="max-w-3xl">
                ...
                <p class="mt-4 text-sm text-slate-300">Accédez rapidement aux échanges importants et retrouvez vos conversations dans un tableau clair.</p>
            </div>
            <div class="inline-flex items-center gap-3 rounded-full bg-white/10 px-5 py-3 text-sm font-semibold text-slate-200">
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="w-full lg:w-96">
                ...
                <div class="relative">
                    <input id="search" type="search" placeholder="Ville, référence, expéditeur" class="w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 pr-12 text-sm text-slate-900 shadow-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-100" />
    ...
    <div class="grid gap-5 xl:grid-cols-2">
        ...
        <a href="{{ route('driver.messages.show', $conversation) }}" class="group flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
            <div class="px-6 py-6">
                <div class="flex items-start gap-4">
                    <div class="flex h-14 w-14 items-center justify-center rounded-3xl bg-white text-primary-600 shadow-sm">
                        <i data-lucide="message-circle" class="w-6 h-6"></i>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs uppercase tracking-[0.28em] text-slate-500">Demande {{ $conversation->demande->reference ?? '—' }}</p>
                        ...
                        <p class="mt-2 text-sm text-slate-600">Expéditeur : {{ $conversation->expediteur->user->prenom ?? '' }} {{ $conversation->expediteur->user->nom ?? '' }}</p>
                    </div>
                </div>
            ...
            <div class="border-t border-slate-200 bg-white px-6 py-4">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between text-xs text-slate-500">
                    <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1"> <i data-lucide="clock" class="w-3.5 h-3.5"></i> {{ $conversation->updated_at->diffForHumans() }}</span>
    ...
    <div class="rounded-3xl border border-slate-200 bg-slate-50 p-12 text-center shadow-sm">
        ...
        <p class="mx-auto max-w-2xl text-sm text-slate-500">Les conversations s’afficheront ici dès qu’un expéditeur prendra contact avec vous. Le design est pensé pour rester clair et efficace.</p>
    </div>
    @endif
</div>
@endsection
