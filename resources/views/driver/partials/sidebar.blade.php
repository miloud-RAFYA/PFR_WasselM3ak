...
>
    <div class="p-4">
        ...
        <!-- Navigation -->
        <nav class="space-y-2">
            <a href="{{ route('driver.dashboard') }}" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl {{ ($active ?? '') === 'dashboard' ? 'bg-primary-500 text-white' : 'text-slate-600 hover:bg-slate-100' }} transition-all">
                <i data-lucide="layout-dashboard" class="w-5 h-5 flex-shrink-0"></i>
                <span x-show="sidebarOpen" class="font-medium">Tableau de bord</span>
            </a>
            <a href="{{ route('driver.available') }}" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl {{ ($active ?? '') === 'available' ? 'bg-primary-500 text-white' : 'text-slate-600 hover:bg-slate-100' }} transition-all">
            ...
            {{-- {{ route('driver.trips') }} --}}
            <a href="" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl {{ ($active ?? '') === 'trips' ? 'bg-primary-500 text-white' : 'text-slate-600 hover:bg-slate-100' }} transition-all">
            ...
            <a href="" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl {{ ($active ?? '') === 'vehicle' ? 'bg-primary-500 text-white' : 'text-slate-600 hover:bg-slate-100' }} transition-all">
            ...
            <a href="" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl {{ ($active ?? '') === 'messages' ? 'bg-primary-500 text-white' : 'text-slate-600 hover:bg-slate-100' }} transition-all">
                <i data-lucide="message-square" class="w-5 h-5 flex-shrink-0"></i>
                <span x-show="sidebarOpen" class="font-medium">Messages</span>
            </a>
            {{-- {{ route('profile') }} --}}
            <a href="" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl {{ ($active ?? '') === 'profile' ? 'bg-primary-500 text-white' : 'text-slate-600 hover:bg-slate-100' }} transition-all">
