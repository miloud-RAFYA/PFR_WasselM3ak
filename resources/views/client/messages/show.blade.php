    ...
    <div class="space-y-6">
        <div class="grid gap-6 lg:grid-cols-[0.95fr_1.7fr]">
            <div class="space-y-6">
                <div class="rounded-3xl border bg-white p-6 shadow-sm">
                    ...
                    <p class="mt-2 text-sm text-slate-500">
                        Transporteur : {{ $conversation->chauffeur->user->prenom }}
                    </p>
                    <div class="mt-4 flex flex-wrap gap-3">
                        <a href="{{ route('client.messages') }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Retour</a>
            ...
            <div class="bg-white p-4 border-t">
                <form id="chat-form" action="{{ route('client.messages.send', $conversation) }}" method="POST">
                    @csrf
...
$(document).ready(function() {
    ...
    const sendRoute = "{{ route('client.messages.send', $conversation) }}";
    ...
    window.Echo.channel('chat.' + conversationId)
        ...
        .listen('user.typing', (e) => {
            ...
            if (e.userName) {
                indicator.querySelector('span').innerHTML = '✏️';
